...
public class SubTileGenerating extends SubTileEntity {
    ...
    public void onUpdate() {
        ...
        if (!supertile.getWorldObj().isRemote) {
            ...
            if (passive && muhBalance > 0 && passiveDecayTicks > muhBalance) {
                ...
                if (supertile.getWorldObj().getBlock(supertile.xCoord, supertile.yCoord - 1, supertile.zCoord)
                        .isSideSolid(
                                supertile.getWorldObj(),
                                supertile.xCoord,
                                supertile.yCoord - 1,
                                ...
                                ForgeDirection.UP))
                    supertile.getWorldObj()
                            .setBlock(supertile.xCoord, supertile.yCoord, supertile.zCoord, Blocks.deadbush);
                else supertile.getWorldObj().setBlockToAir(supertile.xCoord, supertile.yCoord, supertile.zCoord);
            }
        }

        if (!overgrowth && passive) passiveDecayTicks++;
    }
    ...
    public void linkCollector() {
        ...
        if (linkedCollector == null) {
            ...
            TileEntity tileAt = supertile.getWorldObj()
    ...
    public ArrayList<ItemStack> getDrops(ArrayList<ItemStack> list) {
        ...
        if (isPassiveFlower()) populateDropStackNBTs(drops);
        return drops;
    }
    ...
    public boolean bindTo(EntityPlayer player, ItemStack wand, int x, int y, int z, int side) {
        ...
        double dist = (x - supertile.xCoord) * (x - supertile.xCoord) + (y - supertile.yCoord) * (y - supertile.yCoord)
