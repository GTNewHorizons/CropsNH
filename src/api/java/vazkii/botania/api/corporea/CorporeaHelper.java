...
public final class CorporeaHelper {
    ...
    /**
     * Gets a list of all the inventories on this spark network. This list is cached for use once every tick, and if
     * something changes during that tick it'll still have the first result.
     */
    public static List<IInventory> getInventoriesOnNetwork(ICorporeaSpark spark) {
        ...
        if (network != null) for (ICorporeaSpark otherSpark : network) if (otherSpark != null) {
    ...
    /**
     * Gets the amount of available items in the network of the type passed in, checking NBT or not. The higher level
     * functions that use a List< IInventory > or a Map< IInventory, Integer > should be called instead if the context
     * for those exists to avoid having to get the values again.
     */
    ...
    /**
     * Gets the amount of available items in the network of the type passed in, checking NBT or not. The higher level
     * function that use a Map< IInventory, Integer > should be called instead if the context for this exists to avoid
     * having to get the value again.
     */
    ...
    public static int getCountInNetwork(ItemStack stack, Map<IInventory, Integer> inventories, boolean checkNBT) {
        ...
        for (IInventory inv : inventories.keySet()) count += inventories.get(inv);

        return count;
    }

    /**
     * Gets a Map mapping IInventories to the amount of items of the type passed in that exist The higher level function
     * that use a List< IInventory > should be called instead if the context for this exists to avoid having to get the
     * value again.
     */
    public static Map<IInventory, Integer> getInventoriesWithItemInNetwork(ItemStack stack, ICorporeaSpark spark,
            boolean checkNBT) {
        List<IInventory> inventories = getInventoriesOnNetwork(spark);
    ...
    /**
     * Gets a Map mapping IInventories to the amount of items of the type passed in that exist The deeper level function
     * that use a List< IInventory > should be called instead if the context for this exists to avoid having to get the
     * value again.
     */
    public static Map<IInventory, Integer> getInventoriesWithItemInNetwork(ItemStack stack,
            List<IInventory> inventories, boolean checkNBT) {
        ...
        for (IInventory inv : inventories) {
            ...
            for (int i = 0; i < inv.getSizeInventory(); i++) {
                ...
                if (stacksMatch(stack, stackAt, checkNBT)) count += stackAt.stackSize;
            }

            if (count > 0) countMap.put(inv, count);
        }
    ...
    /**
     * Requests list of ItemStacks of the type passed in from the network, or tries to, checking NBT or not. This will
     * remove the items from the adequate inventories unless the "doit" parameter is false. Returns a new list of
     * ItemStacks of the items acquired or an empty list if none was found. Case itemCount is -1 it'll find EVERY item
     * it can. <br>
     * <br>
     * The "matcher" parameter has to be an ItemStack or a String, if the first it'll check if the two stacks are
     * similar using the "checkNBT" parameter, else it'll check if the name of the item equals or matches (case a regex
     * is passed in) the matcher string.
     */
    public static List<ItemStack> requestItem(Object matcher, int itemCount, ICorporeaSpark spark, boolean checkNBT,
        ...
        for (IInventory inv : inventories) {
            ...
            for (int i = inv.getSizeInventory() - 1; i >= 0; i--) {
                ...
                ItemStack stackAt = inv.getStackInSlot(i);
                if (matcher instanceof ItemStack ? stacksMatch((ItemStack) matcher, stackAt, checkNBT)
                        : matcher instanceof String ? stacksMatch(stackAt, (String) matcher) : false) {
                    int rem = Math.min(stackAt.stackSize, count == -1 ? stackAt.stackSize : count);

                    if (rem > 0) {
                        ItemStack copy = stackAt.copy();
                        if (rem < copy.stackSize) copy.stackSize = rem;
                        stacks.add(copy);
                    }
                    ...
                    if (doit && rem > 0) {
                        ...
                        removedAny = true;
                        if (invSpark != null) invSpark.onItemExtracted(stackAt);
                    }
                    if (count != -1) count -= rem;
                }
            }

            if (removedAny) inv.markDirty();
        }

        for (ICorporeaInterceptor interceptor : interceptors.keySet()) interceptor.interceptRequestLast(
                matcher,
                itemCount,
                interceptors.get(interceptor),
                spark,
                stacks,
                inventories,
                doit);
    ...
    public static ICorporeaSpark getSparkForInventory(IInventory inv) {
        if (!(inv instanceof TileEntity)) return null;
    ...
    /**
     * Gets the spark attached to the block in the coords passed in. Note that the coords passed in are for the block
     * that the spark will be on, not the coords of the spark itself.
     */
    public static ICorporeaSpark getSparkForBlock(World world, int x, int y, int z) {
        List<ICorporeaSpark> sparks = world.getEntitiesWithinAABB(
                ICorporeaSpark.class,
                AxisAlignedBB.getBoundingBox(x, y + 1, z, x + 1, y + 2, z + 1));
        return sparks.isEmpty() ? null : sparks.get(0);
    }

    /**
     * Gets if the block in the coords passed in has a spark attached. Note that the coords passed in are for the block
     * that the spark will be on, not the coords of the spark itself.
     */
    ...
    public static boolean isValidSlot(IInventory inv, int slot) {
        return !(inv instanceof ISidedInventory)
                || arrayHas(((ISidedInventory) inv).getAccessibleSlotsFromSide(ForgeDirection.UP.ordinal()), slot)
                        && ((ISidedInventory) inv)
                                .canExtractItem(slot, inv.getStackInSlot(slot), ForgeDirection.UP.ordinal());
    }
