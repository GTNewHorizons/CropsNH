/**
 * This class was created by <Vazkii>. It's distributed as part of the Botania Mod. Get the Source Code in github:
 * https://github.com/Vazkii/Botania
 * 
 * Botania is Open Source and distributed under the Botania License: http://botaniamod.net/license.php
 * 
 * File Created @ [Feb 22, 2015, 2:02:02 PM (GMT)]
 */
package vazkii.botania.api.item;

/**
 * A bauble that counts as a cosmetic only item. These can be added to other baubles to add the render to them. Other
 * cosmetic baubles can't be stacked on this.
 */
public interface ICosmeticBauble extends IBaubleRender {

}
