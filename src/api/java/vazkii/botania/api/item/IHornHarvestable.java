...
public interface IHornHarvestable {
    ...
    /**
     * Returns true if this block can be uprooted. Note that the stack param can be null if it's a drum breaking it.
     */
    boolean canHornHarvest(World world, int x, int y, int z, ItemStack stack, EnumHornType hornType);

    /**
     * Returns true if harvestByHorn() should be called. If false it just uses the normal block breaking method. Note
     * that the stack param can be null if it's a drum breaking it.
     */
    boolean hasSpecialHornHarvest(World world, int x, int y, int z, ItemStack stack, EnumHornType hornType);

    /**
     * Called to harvest by a horn. Note that the stack param can be null if it's a drum breaking it.
     */
