...
public final class ManaItemHandler {
     ...
     * 
     * @param manaToGet How much mana is to be requested, if less mana exists than this amount, the amount of mana
     *                  existent will be returned instead, if you want exact values use requestManaExact.
            ...
            if (stackInSlot != null && stackInSlot.getItem() instanceof IManaItem) {
                ...
                if (manaItem.canExportManaToItem(stackInSlot, stack) && manaItem.getMana(stackInSlot) > 0) {
                    if (stack.getItem() instanceof IManaItem
                            && !((IManaItem) stack.getItem()).canReceiveManaFromItem(stack, stackInSlot))
                        continue;
     ...
     * 
     * @param manaToGet How much mana is to be requested, if less mana exists than this amount, false will be returned
    ...
    /**
     * Dispatches mana to items in a given player's inventory. Note that this method does not automatically remove mana
     * from the item which is exporting.
     * 
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (stackInSlot != null && stackInSlot.getItem() instanceof IManaItem) {
                ...
                if (manaItemSlot.canReceiveManaFromItem(stackInSlot, stack)) {
                    if (stack.getItem() instanceof IManaItem
                            && !((IManaItem) stack.getItem()).canExportManaToItem(stack, stackInSlot))
                        continue;
    ...
    /**
     * Dispatches an exact amount of mana to items in a given player's inventory. Note that this method does not
     * automatically remove mana from the item which is exporting.
     * 
     * @param manaToSend How much mana is to be sent.
     * @param remove     If true, the mana will be added from the target item. Set to false to just check.
    ...
    /**
     * Requests mana from items in a given player's inventory. This version also checks for IManaDiscountArmor items
     * equipped to lower the cost.
     * 
     * @param manaToGet How much mana is to be requested, if less mana exists than this amount, the amount of mana
     *                  existent will be returned instead, if you want exact values use requestManaExact.
     * @param remove    If true, the mana will be removed from the target item. Set to false to just check.
    ...
    /**
     * Requests an exact amount of mana from items in a given player's inventory. This version also checks for
     * IManaDiscountArmor items equipped to lower the cost.
     * 
     * @param manaToGet How much mana is to be requested, if less mana exists than this amount, false will be returned
     *                  instead, and nothing will happen.
     * @param remove    If true, the mana will be removed from the target item. Set to false to just check.
    ...
    /**
     * Gets the sum of all the discounts on IManaDiscountArmor items equipped on the player passed in.
     */
