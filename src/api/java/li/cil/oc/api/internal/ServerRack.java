 ...
 * </pre>
 * <p/>
 * Server racks <em>do not</em> serve as environment for the computer nodes of servers. That's what the
 * {@link li.cil.oc.api.internal.Server}s are for.
 * <p/>
 * The only purpose is to allow identifying tile entities as server racks via the API, i.e. without having to link
 * against internal classes. This also means that <em>you should not implement this</em>.
 */
public interface ServerRack extends Environment, SidedEnvironment, Rotatable, IInventory {
     ...
     * <p/>
     * This can be <tt>null</tt>, for example when there is no server installed in that slot.
     *
