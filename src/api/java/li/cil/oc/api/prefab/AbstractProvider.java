 ...
 * <p/>
 * This class takes care of handling the unique identifier used to tell if a behavior is its own when loading from NBT.
 */
public abstract class AbstractProvider implements BehaviorProvider {
    ...
    /**
     * For the ID passed in here, it is suggested to use a one-time generated UUID that is hard-coded into your provider
     * implementation. Take care to use a different one for each provider you create!
     *
     ...
     * <p/>
     * The ID will already have been written, don't overwrite it. Store any additional data you need to restore the
     * behavior here, if any.
     *
     ...
     * <p/>
     * Use the data written in {@link #writeBehaviorToNBT} to restore the behavior to its previous state, then return
     * it.
     *
