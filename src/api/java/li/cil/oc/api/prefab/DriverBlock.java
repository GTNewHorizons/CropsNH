...
/**
 * If you wish to create a block component for a third-party block, i.e. a block for which you do not control the tile
 * entity, such as vanilla blocks, you will need a block driver.
 * <p/>
 * This prefab allows creating a driver that works for a specified list of item stacks (to support different blocks with
 * the same id but different metadata values).
 * <p/>
    ...
    protected boolean worksWith(final Block referenceBlock, final int referenceMetadata) {
        for (ItemStack stack : blocks) {
            if (stack != null && stack.getItem() instanceof ItemBlock) {
                ...
                if (referenceBlock == supportedBlock && (referenceMetadata == supportedMetadata
