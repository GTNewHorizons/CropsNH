...
public class RobotUsedToolEvent extends RobotEvent {
    ...
    /**
     * The rate at which the used tool should lose durability, where one means it loses durability at full speed, zero
     * means it doesn't lose durability at all.
     * <p/>
    ...
    /**
     * Fired when a robot used a tool and is about to apply the damage rate to partially undo the durability loss. This
     * step is used to compute the rate at which the tool should lose durability, which is used by the experience
     * upgrade, for example.
     */
    ...
    /**
     * Fired when a robot used a tool and the previously fired damage rate computation returned a value smaller than
     * one. The callbacks of this method are responsible for applying the inverse damage the tool took. The
     * <tt>toolAfterUse</tt> item stack represents the actual tool, any changes must be applied to that variable. The
     * <tt>toolBeforeUse</tt> item stack is passed for reference, to compute the actual amount of durability that was
     * lost. This may be required for tools where the durability is stored in the item's NBT tag.
     */
