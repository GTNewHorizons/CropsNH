 ...
 * <p/>
 * Note that the item must be installed in the actual server's inventory to work. If it is installed in an external
 * inventory the server will not recognize the memory.
 */
public interface Processor extends Item {
     ...
     * <p/>
     * This usually controls which architecture is created for a machine the CPU is installed in (this is true for all
     * computers built into OC, such as computer cases, server racks and robots, it my not be true for third- party
     * computers).
     *
