...
public class ItemHelper {
    ...
    public static ItemStack getOreStack(String entry) {
        if (OreDictionary.getOreNames().length != 0 && OreDictionary.doesOreNameExist(entry)) {
            if (OreDictionary.getOres(entry).size() != 0) return OreDictionary.getOres(entry).get(0);
            else return new ItemStack(Blocks.fire);
        } else return new ItemStack(Blocks.fire);
    }

    /**
     * Takes a string input with a specific formatting and parses it as an ItemStack.
     *
     * Syntax: domain:regname:meta#amount IE: minecraft:stone:0#8
     *
    ...
    public static ItemStack parseItemStack(String stackString, boolean input) {
        ...
        try {
            ...
            // LogHelper.error("Error adding " + (input ? "inputStack" : "outputStack") + ": " + stackString + ". Is it
            // formatted correctly?");
        }
