...
public class ThaumcraftApi {
    ...
    // Miscellaneous
    /**
     * Portable Hole Block-id Blacklist. Simply add the block-id's of blocks you don't want the portable hole to go
     * through.
     */
    ...
    /**
     * This is used to add aspects to entities which you can then scan using a thaumometer. Also used to calculate vis
     * drops from mobs.
     * 
     * @param entityName
     * @param aspects
     * @param nbt        you can specify certain nbt keys and their values to differentiate between mobs. <br>
    ...
    private static final HashMap<Object, ItemStack> smeltingBonus = new HashMap<>();
     ...
     *                    the central item
     * @param instability a number that represents the N in 1000 chance for the infusion altar to spawn an instability
     *                    effect each second while the crafting is in progress
     * @param aspects     the essentia cost per aspect.
     * @param aspects     input the central item to be infused
     * @param recipe      An array of items required to craft this. Input itemstacks are NBT sensitive. Infusion
     *                    crafting components are automatically "fuzzy" and the oredict will be checked for possible
     *                    matches.
     *
     */
     ...
     */
    private static final HashMap<int[], Object[]> keyCache = new HashMap<>();
    ...
    /**
     * Used to assign aspects to the given item/block. Attempts to automatically generate aspect tags by checking
     * registered recipes. Here is an example of the declaration for pistons:
     * <p>
     * <i>ThaumcraftApi.registerComplexObjectTag(new ItemStack(Blocks.cobblestone), (new
     * AspectList()).add(Aspect.MECHANISM, 2).add(Aspect.MOTION, 4));</i> IMPORTANT - this should only be used if you
     * are not happy with the default aspects the object would be assigned.
     * 
    ...
    // WARP ///////////////////////////////////////////////////////////////////////////////////////
    private static final HashMap<Object, Integer> warpMap = new HashMap<>();

    /**
     * This method is used to determine how much warp is gained if the item is crafted. The warp added is "sticky" warp
     * 
    ...
    /**
     * Used to add possible loot to treasure bags. As a reference, the weight of gold coins are 2000 and a diamond is
     * 50. The weights are the same for all loot bag types - the only difference is how many items the bag contains.
     * 
     * @param item
     * @param weight
     * @param bagTypes array of which type of bag to add this loot to. Multiple types can be specified 0 = common, 1 =
     *                 uncommon, 2 = rare
     */
    ...
    /**
     * To define mod crops you need to use FMLInterModComms in your @Mod.Init method. There are two 'types' of crops you
     * can add. Standard crops and clickable crops.
     *
     * Standard crops work like normal vanilla crops - they grow until a certain metadata value is reached and you
     * harvest them by destroying the block and collecting the blocks. You need to create and ItemStack that tells the
     * golem what block id and metadata represents the crop when fully grown. Sending a metadata of
     * [OreDictionary.WILDCARD_VALUE] will mean the metadata won't get checked. Example for vanilla wheat:
     * FMLInterModComms.sendMessage("Thaumcraft", "harvestStandardCrop", new ItemStack(Block.crops,1,7));
     *
     * Clickable crops are crops that you right click to gather their bounty instead of destroying them. As for standard
     * crops, you need to create and ItemStack that tells the golem what block id and metadata represents the crop when
     * fully grown. The golem will trigger the blocks onBlockActivated method. Sending a metadata of
     * [OreDictionary.WILDCARD_VALUE] will mean the metadata won't get checked. Example (this will technically do
     * nothing since clicking wheat does nothing, but you get the idea): FMLInterModComms.sendMessage("Thaumcraft",
     * "harvestClickableCrop", new ItemStack(Block.crops,1,7));
     *
     * Stacked crops (like reeds) are crops that you wish the bottom block should remain after harvesting. As for
     * standard crops, you need to create and ItemStack that tells the golem what block id and metadata represents the
     * crop when fully grown. Sending a metadata of [OreDictionary.WILDCARD_VALUE] will mean the actualy md won't get
     * checked. If it has the order upgrade it will only harvest if the crop is more than one block high. Example:
    ...
    /**
     * You can define certain ores that will have a chance to produce native clusters via FMLInterModComms in
     * your @Mod.Init method using the "nativeCluster" string message. The format should be: "[ore item/block id],[ore
     * item/block metadata],[cluster item/block id],[cluster item/block metadata],[chance modifier float]"
     *
     ...
     * default chance: FMLInterModComms.sendMessage("Thaumcraft", "nativeCluster","15,0,25016,16,2.0");
     */

    // LAMP OF GROWTH BLACKLIST ///////////////////////////////////////////////////////////////////////////
    /**
     * You can blacklist crops that should not be effected by the Lamp of Growth via FMLInterModComms in your @Mod.Init
     * method using the "lampBlacklist" itemstack message. Sending a metadata of [OreDictionary.WILDCARD_VALUE] will
     * mean the metadata won't get checked. Example for vanilla wheat: FMLInterModComms.sendMessage("Thaumcraft",
     * "lampBlacklist", new ItemStack(Block.crops,1,OreDictionary.WILDCARD_VALUE));
     */

    // DIMENSION BLACKLIST ///////////////////////////////////////////////////////////////////////////
    /**
     * You can blacklist a dimension to not spawn certain thaumcraft features in your @Mod.Init method using the
     * "dimensionBlacklist" string message in the format "[dimension]:[level]" The level values are as follows: [0] stop
     * all tc spawning and generation [1] allow ore and node generation (and node special features) [2] allow mob
     * spawning [3] allow ore and node gen + mob spawning (and node special features) Example:
     * FMLInterModComms.sendMessage("Thaumcraft", "dimensionBlacklist", "15:1");
     */

    // BIOME BLACKLIST ///////////////////////////////////////////////////////////////////////////
    /**
     * You can blacklist a biome to not spawn certain thaumcraft features in your @Mod.Init method using the
     * "biomeBlacklist" string message in the format "[biome id]:[level]" The level values are as follows: [0] stop all
     * tc spawning and generation [1] allow ore and node generation (and node special features) [2] allow mob spawning
     * [3] allow ore and node gen + mob spawning (and node special features) Example:
