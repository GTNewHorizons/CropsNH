 ...
 * <p>
 * CableSingle: Connections will use the cable renderer with a single band, best used for whole blocks <br>
 * PlateSingle: Connections will use the plate renderer with a single band, used for conveyers and rails
 * <p>
 * CableAll: Connections permit access to all 16 bands <br>
 * PlateAll: Connections permit access to all 16 bands
 * <p>
 * <p>
 * Forced connection modes are best used for decoration blocks: RedNet will not connect normally, but will if the user
 * forces it. Typically, IRedNetDecorative is desired for this instead
 * <p>
 * ForcedCableSingle: Connections permit access to a single band, only when the cable is in forced connection mode <br>
 * ForcedPlateSingle: Connections permit access to a single band, only when the cable is in forced connection mode
 * <p>
 * ForcedCableAll: Connections permit access to all 16 bands, only when the cable is in forced connection mode <br>
 * ForcedPlateAll: Connections permit access to all 16 bands, only when the cable is in forced connection mode
 * <p>
 * <p>
 * The decorative nodes are for when you want rednet to decide how to connect to your block, but also need to receive
 * full updates from the network.
 * <p>
 * DecorativeSingle: Connections permit access to a single band, using standard connection logic <br>
 * DecorativeAll: Connections permit access to all 16 bands, using standard connection logic <br>
