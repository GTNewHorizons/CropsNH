...
public class MutateStrategy extends BaseStrategy {
    ...
    public ICrossOverResult executeStrategy() {
        List<ICrop> neighbours = engine.getCrop()
            .getMatureNeighbours();
        IMutation[] crossOvers = MutationHandler.getInstance()
            .getCrossOvers(neighbours);
        if (crossOvers.length > 0) {
            int index = engine.getRandom()
                .nextInt(crossOvers.length);
            if (crossOvers[index].getResult()
                .getItem() != null) {
    ...
    private CrossOverResult fromMutation(IMutation mutation, List<ICrop> neighbours) {
        Item seed = mutation.getResult()
            .getItem();
        int meta = mutation.getResult()
            .getItemDamage();
        ...
        return new CrossOverResult(
            ...
            meta,
            mutation.getChance(),
            engine.getStatCalculator()
                .calculateStats(seed, meta, neighbours, true));
    }
}
