...
public abstract class StatCalculatorBase extends StatCalculator {
    ...
    public ISeedStats calculateStats(Item result, int resultMeta, List<? extends ICrop> input, boolean mutation) {
        ...
        for (int i = 0; i < nrParents; i++) {
            boolean canInherit = canInheritStats(
                result,
                resultMeta,
                parents[i].getSeedStack()
                    .getItem(),
                parents[i].getSeedStack()
                    .getItemDamage());
            if (canInherit) {
            ...
            // -1: if neighbour is a non-parent crops and non parent crops do not affect stats, multiplier is -1
            // (negative values get filtered)
            // 0 : if neighbour is a non-parent crop, and non parent crops affect stat gain negatively, multiplier is 0
            // (0 will reduce the average)
    ...
    protected boolean canInheritStats(Item child, int childMeta, Item seed, int seedMeta) {
        ...
        if (!b) {
            for (IMutation mutation : MutationHandler.getInstance()
                .getMutationsFromChild(child, childMeta)) {
                if (mutation != null) {
