...
public class SpreadStrategy extends BaseStrategy {
    ...
    public CrossOverResult executeStrategy() {
        List<ICrop> matureNeighbours = engine.getCrop()
            .getMatureNeighbours();
        if (matureNeighbours.isEmpty()) {
            return null;
        }
        int index = engine.getRandom()
            .nextInt(matureNeighbours.size());
    ...
    private CrossOverResult fromTileEntityCrop(ICrop crop, List<ICrop> neighbours) {
        Item seed = crop.getSeedStack()
            .getItem();
        int meta = crop.getSeedStack()
            .getItemDamage();
        double chance = ((double) crop.getPlant()
            .getSpreadChance()) / 100.0;
        ...
        return new CrossOverResult(
            ...
            engine.getStatCalculator()
                .calculateStats(seed, meta, neighbours, false));
    }
}
