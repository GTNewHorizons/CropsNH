...
public class CropPlantHandler {
     ...
     * on code duplication. Furthermore, this is a targeted switch statement, instead of a catch all, allowing
     * exceptions that should not have been thrown to surface properly.
     *
    ...
    private static void removeFromBlackListArray(ItemStack seed) {
        blacklist.removeIf(
                queued -> seed.getItem() == queued.getItem() && seed.getItemDamage() == queued.getItemDamage());
    }
     ...
     * queue. Finally initializes plants found in the ore dictionary.
     */
