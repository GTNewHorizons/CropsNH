...
public class TileEntitySeedStorage extends TileEntityCustomWood
    implements ISeedStorageControllable, IDebuggable, ISidedInventory {
            ...
            if (tag.hasKey(Names.NBT.inventory)) {
                ...
                for (int i = 0; i < tagList.tagCount(); i++) {
                    ...
                    CropRegistry.setSeedNBT(
                        ...
                        slotTag.getShort(Names.NBT.strength),
                        true);
                    int id = slotTag.getInteger(Names.NBT.id);
                    SeedStorageSlot slot = new SeedStorageSlot(
                        stackTag,
                        slotTag.getInteger(Names.NBT.count),
                        id,
                        invId);
                    slots.put(id, slot);
                    slotsList.add(slot);
    ...
    public void syncSlotToClient(SeedStorageSlot slot) {
        NetworkWrapperCropsNH.wrapper.sendToDimension(
            new MessageTileEntitySeedStorage(this.xCoord, this.yCoord, this.zCoord, slot),
    ...
    public void addDebugInfo(List<String> list) {
        String info = this.lockedSeed == null ? "null"
            : this.getLockedSeed()
                .getDisplayName();
    ...
    public void addWailaInformation(List information) {
        information.add(
            StatCollector.translateToLocal("cropsnh_tooltip.storage") + ": "
                + (this.hasLockedSeed() ? getLockedSeed().getDisplayName()
    ...
    public boolean addStackToInventory(ItemStack stack) {
        ...
        if (!this.worldObj.isRemote) {
            if (this.hasLockedSeed() && this.lockedSeed == stack.getItem()
                && this.lockedSeedMeta == stack.getItemDamage()) {
                for (Map.Entry<Integer, SeedStorageSlot> entry : this.slots.entrySet()) {
                    if (entry.getValue() != null) {
                        if (ItemStack.areItemStackTagsEqual(
                            entry.getValue()
                                .getStack(this.lockedSeed, this.lockedSeedMeta),
                            stack)) {
    ...
    public void setSlotContents(int realSlotId, ItemStack inputStack) {
        ...
        if (this.isValidForSlot(realSlotId, inputStack)) {
            ...
            if (slotAt != null) {
                ...
                slotAt = new SeedStorageSlot(
                    inputStack.getTagCompound(),
                    inputStack.stackSize,
                    realSlotId,
                    this.getControllableID());
                if (slotAt.count > 0) {
    ...
    private boolean isValidForSlot(int realSlot, ItemStack stack) {
        ...
        if (this.hasLockedSeed()) {
            if (stack.getItem() == this.lockedSeed && stack.getItemDamage() == this.lockedSeedMeta) {
                ...
                return slotAt == null
                    || ItemStack.areItemStackTagsEqual(stack, slotAt.getStack(lockedSeed, lockedSeedMeta));
            }
        } else {
            return true;
        }
    ...
    // INVENTORY METHODS
    // -----------------
    // TODO: Fix these methods to work with slot ID's from the ArrayList and then forward them to the
    // ISeedStorageControllable methods with the correct ID's
    ...
    public ItemStack getStackInSlot(int slot) {
        ...
        return slotsList.get(slot)
            .getStack(lockedSeed, lockedSeedMeta);
    }
    ...
    public boolean canInsertItem(int slot, ItemStack stack, int side) {
        ...
        if (slot >= slotsList.size()) {
            // 0 is a virtual slot only used for inputs, this is a workaround to prevent derpy behaviour with code which
            // modifies stacks in slots directly
            return (!this.hasLockedSeed())
                || (stack != null && stack.getItem() == lockedSeed && stack.getItemDamage() == lockedSeedMeta);
        }
        return false;
    }
    ...
    public boolean canExtractItem(int slot, ItemStack stack, int side) {
        ...
        return ItemStack.areItemStackTagsEqual(
            stack,
            slotsList.get(slot)
                .getStack(lockedSeed, lockedSeedMeta));
    }
