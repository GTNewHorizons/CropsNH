    ...
    private void registerSeedOilRecipes() {
        ...
        for (ItemModSeed seed : Crops.seeds) {
            RecipeManagers.squeezerManager
                .addRecipe(10, new ItemStack[] { new ItemStack(seed, 1, 0) }, FluidRegistry.getFluidStack(seedOil, 10));
        }
        // resource crops
        if (ConfigurationHandler.resourcePlants) {
            for (ItemModSeed seed : ResourceCrops.vanillaSeeds) {
                RecipeManagers.squeezerManager.addRecipe(
                    10,
                    new ItemStack[] { new ItemStack(seed, 1, 0) },
                    FluidRegistry.getFluidStack(seedOil, 10));
            }
        ...
        if (ModHelper.allowIntegration(Names.Mods.thaumcraft)) {
            for (ItemModSeed seed : ThaumcraftHelper.thaumcraftSeeds) {
                if (seed.getUnlocalizedName()
                    .equals("cropsnh:seedTaintedRoot")) {
                    RecipeManagers.squeezerManager.addRecipe(
                        10,
                        new ItemStack[] { new ItemStack(seed, 1, 0) },
                        FluidRegistry.getFluidStack(seedOil, 10));
