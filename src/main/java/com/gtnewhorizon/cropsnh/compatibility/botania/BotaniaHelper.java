...
public class BotaniaHelper extends ModHelper {
    ...
    protected void initPlants() {
        for (int i = 0; i < 16; i++) {
            Object[] args = { Data.botania[i], new ItemStack((Item) Item.itemRegistry.getObject("Botania:petal"), 1, i),
                new BlockWithMeta(Blocks.dirt, 2), 3, RenderMethod.CROSSED,
                new ItemStack((Block) Block.blockRegistry.getObject("Botania:flower"), 1, i) };
            BlockModPlant plant;
            try {
    ...
    static boolean useAlternateTextures() {
        try {
            ...
            return (Boolean) configClass.getField("altFlowerTextures")
                .get(null);
        } catch (Exception e) {
