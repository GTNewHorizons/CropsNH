package com.gtnewhorizon.cropsnh.compatibility.applecore;
...
public final class AppleCoreHelper extends ModHelper {
    ...
    private static Event.Result validateAppleCoreGrowthTick(Block block, World world, int x, int y, int z,
        Random random) {
    ...
    public static Event.Result validateGrowthTick(Block block, World world, int x, int y, int z, Random random) {
        if (hasDispatcher) return validateAppleCoreGrowthTick(block, world, x, y, z, random);
        else return Event.Result.DEFAULT;
    }
