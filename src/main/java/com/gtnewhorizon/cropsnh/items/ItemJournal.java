...
public class ItemJournal extends ItemCropsNH implements IJournal {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean flag) {
        ...
        if (stack.hasTagCompound() && stack.stackTagCompound.hasKey(Names.NBT.discoveredSeeds)) {
            nr = stack.stackTagCompound.getTagList(Names.NBT.discoveredSeeds, 10)
                .tagCount();
        }
        list.add(StatCollector.translateToLocal("cropsnh_tooltip.discoveredSeeds") + ": " + nr);
    }
    ...
    public ArrayList<ItemStack> getDiscoveredSeeds(ItemStack journal) {
        ...
        if (journal != null && journal.stackSize > 0
            && journal.getItem() instanceof ItemJournal
            && journal.hasTagCompound()) {
