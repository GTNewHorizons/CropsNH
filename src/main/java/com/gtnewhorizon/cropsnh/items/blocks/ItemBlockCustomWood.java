 ...
 */
public class ItemBlockCustomWood extends ItemBlockCropsNH {
    ...
    /**
     * Populates the sub-item list. This method allows getting sub blocks server side as well (no @side, like
     * {@link #getSubItems(Item, CreativeTabs, List)}). This method is marked for cleaning.
     *
        ...
        for (ItemStack plank : planks) {
            if (plank.getItem() instanceof ItemBlock) {
                ...
                if (Loader.isModLoaded(Names.Mods.extraUtilities)
                        && ((ItemBlock) plank.getItem()).field_150939_a.getClass().getSimpleName().equals("BlockColor"))
                    continue;
    ...
    /**
     * Retrieves the block's displayable information. This method does not need to be overridden by most CustomWood
     * blocks.
     * <p>
     * If the block name is not displaying correctly, check the language files and Names.Objects.[blockname]. If that
     * does not correct the issue, ensure that the block overrides both getInternalName() and getTileEntityName() and
     * returns Names.Objects.[blockname].
     * </p>
     * <p>
     * All custom wood blocks have a material that we want shown, so we make this method final. Some however, has more
     * information they want to add, so we add a addMore() method to override in that event.
     * </p>
     */
    @SideOnly(Side.CLIENT)
    @SuppressWarnings("unchecked")
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean flag) {
        ...
        if (stack.getItemDamage() == 0 && stack.hasTagCompound()
                && stack.getTagCompound().hasKey(Names.NBT.material)
                && stack.getTagCompound().hasKey(Names.NBT.materialMeta)) {
        ...
        list.add(
                StatCollector.translateToLocal("cropsnh_tooltip.material") + ": "
                        + material.getItem().getItemStackDisplayName(material));
    }

    /**
     * Retrieves the item's unlocalized name. This is the key used in the language files. Should return something like
     * tile.cropsnh:[internalname].[meta].name Final as to prevent being messed up.
     *
