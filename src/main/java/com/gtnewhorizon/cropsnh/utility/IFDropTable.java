...
public class IFDropTable {
     ...
     */
    public IFDropTable(@NotNull NBTTagCompound nbt, String key) {
     ...
     * @param nbt The nbt tag that contains the key for a drop table
     */
    public IFDropTable(@NotNull NBTTagList nbt) {
        this();
     ...
     */
    public void addTo(@NotNull IFDropTable target, double variance, @NotNull Random rand) {
     ...
     * @param rand       The random source for the variance.
     */
    public void addTo(@NotNull IFDropTable target, double multiplier, double variance, @NotNull Random rand) {
     ...
     * @param target The drop table that you want to add the value to.
     */
    public void addTo(@NotNull IFDropTable target) {
     ...
     * @param multiplier A multiplier to apply to all amounts from this drop table.
     */
    public void addTo(@NotNull IFDropTable target, double multiplier) {
        for (Map.Entry<ItemStack, Double> entry : this.dropTable.entrySet()) {
     ...
     */
    public @NotNull IFDropTable intersect(@NotNull IFDropTable with) {
        IFDropTable ret = new IFDropTable();
     ...
     */
    private static @NotNull ItemStack computeDrops(Map.@NotNull Entry<ItemStack, Double> entry, boolean simulate) {
        ItemStack copied = entry.getKey()
            .copy();
        copied.stackSize = (int) Math.floor(entry.getValue());
        if (!simulate && entry.getValue() >= 1.0d) {
