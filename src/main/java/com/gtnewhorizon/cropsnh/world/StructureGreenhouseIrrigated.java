package com.gtnewhorizon.cropsnh.world;
...
import com.gtnewhorizon.cropsnh.tileentity.irrigation.TileEntityChannel;
import com.gtnewhorizon.cropsnh.tileentity.irrigation.TileEntityTank;
...
public class StructureGreenhouseIrrigated extends StructureGreenhouse {
    ...
    public static StructureGreenhouseIrrigated buildComponent(StructureVillagePieces.Start villagePiece, List pieces, Random random, int p1, int p2, int p3, int p4, int p5) {
        ...
        return (canVillageGoDeeper(boundingBox)) && (findIntersecting(pieces, boundingBox) == null) ? new StructureGreenhouseIrrigated(villagePiece, p5, random, boundingBox, p4) : null;
    }
    ...
    public boolean addComponentParts(World world, Random rand, StructureBoundingBox boundingBox) {
        ...
        this.fillWithBlocks(world, boundingBox, 5, 5, 5, 5, 5, 10, com.gtnewhorizon.cropsnh.init.Blocks.blockWaterChannel, com.gtnewhorizon.cropsnh.init.Blocks.blockWaterChannel, false);
        for(int z=5;z<=10;z++) {
            this.generateStructureIrrigationChannel(world, boundingBox, 5, 5, z);
        }
        this.fillWithBlocks(world, boundingBox, 6, 5, 10, 11, 5, 10, com.gtnewhorizon.cropsnh.init.Blocks.blockWaterChannel, com.gtnewhorizon.cropsnh.init.Blocks.blockWaterChannel, false);
        for(int x=6;x<=11;x++) {
    ...
    protected boolean generateStructureWoodenTank(World world, StructureBoundingBox boundingBox, int x, int y, int z, boolean multiBlockify) {
        ...
        if (boundingBox.isVecInside(xCoord, yCoord, zCoord)) {
            world.setBlock(xCoord, yCoord, zCoord, com.gtnewhorizon.cropsnh.init.Blocks.blockWaterTank, 0, 2);
    ...
    protected boolean generateStructureIrrigationChannel(World world, StructureBoundingBox boundingBox, int x, int y, int z) {
        ...
        int zCoord = this.getZWithOffset(x, z);
        if (boundingBox.isVecInside(xCoord, yCoord, zCoord)) {
            world.setBlock(xCoord, yCoord, zCoord, com.gtnewhorizon.cropsnh.init.Blocks.blockWaterChannel, 0, 2);
    ...
    protected boolean generateStructureSprinkler(World world, StructureBoundingBox boundingBox, int x, int y, int z) {
        ...
        int zCoord = this.getZWithOffset(x, z);
        if (boundingBox.isVecInside(xCoord, yCoord, zCoord)) {
            world.setBlock(xCoord, yCoord, zCoord, com.gtnewhorizon.cropsnh.init.Blocks.blockSprinkler, 0, 2);
            return true;
        }
        else {
            return false;
