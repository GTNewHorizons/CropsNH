...
public class JournalPageSeed extends JournalPage {
        ...
        allMutations.addAll(
            Arrays.asList(
                MutationHandler.getInstance()
                    .getMutationsFromParent(discoveredSeeds.get(page))));
    ...
    private ArrayList<IMutation> getDiscoveredChildMutations() {
        ...
        allMutations.addAll(
            Arrays.asList(
                MutationHandler.getInstance()
                    .getMutationsFromChild(discoveredSeeds.get(page))));
        for (IMutation mutation : allMutations) {
            if (isMutationDiscovered(mutation)) {
                mutations.add(mutation);
            }
    ...
    private Component<ResourceLocation> getSoil() {
        ...
        if (soil != null) {
            texture = getBlockResource(
                soil.getBlock()
                    .getIcon(1, soil.getMeta()));
        } else {
    ...
    private ArrayList<Component<ResourceLocation>> getBrightnessTextures() {
        ...
        int u = 4;
        int v = 8;
        int[] brightnessRange = plant.getGrowthRequirement()
            .getBrightnessRange();
