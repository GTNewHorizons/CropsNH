...
public abstract class GuiSeedStorageBase extends GuiContainer {
    ...
    public GuiSeedStorageBase(ContainerSeedStorageBase container, int maxVertSlots, int maxHorSlots, int sortButtonX,
        int sortButtonY, int setActiveSeedButtonsX, int setActiveSeedButtonsY, int seedSlotsX, int seedSlotsY) {
        super(container);
    ...
    protected void loadButtons() {
        ...
        this.buttonList.add(
            new GuiButton(
                buttonIdGrowth,
                this.guiLeft + sortButtonX,
                this.guiTop + sortButtonY,
                buttonWidth,
                buttonHeight,
        ...
        this.buttonList.add(
            new GuiButton(
                buttonIdGain,
                this.guiLeft + sortButtonX,
                this.guiTop + sortButtonY + buttonHeight + 1,
                buttonWidth,
                buttonHeight,
        ...
        this.buttonList.add(
            new GuiButton(
                buttonIdStrength,
                this.guiLeft + sortButtonX,
                this.guiTop + sortButtonY + 2 * (buttonHeight + 1),
                buttonWidth,
                buttonHeight,
        ...
        this.buttonList.add(
            new GuiButton(
                buttonIdLeftEnd,
                this.guiLeft + sortButtonX,
                this.guiTop + sortButtonY + 3 * (buttonHeight + 1),
                -1 + (buttonWidth) / 4,
                buttonHeight,
                "<<"));
        this.buttonList.add(
            new GuiButton(
                buttonIdScrollLeft,
                this.guiLeft + sortButtonX + (buttonWidth) / 4,
                this.guiTop + sortButtonY + 3 * (buttonHeight + 1),
                -1 + (buttonWidth) / 4,
                buttonHeight,
                "<"));
        this.buttonList.add(
            new GuiButton(
                buttonIdScrollRight,
                this.guiLeft + sortButtonX + 1 + 2 * (buttonWidth) / 4,
                this.guiTop + sortButtonY + 3 * (buttonHeight + 1),
                -1 + (buttonWidth) / 4,
                buttonHeight,
                ">"));
        this.buttonList.add(
            new GuiButton(
                buttonIdRightEnd,
                this.guiLeft + sortButtonX + 1 + 3 * (buttonWidth) / 4,
                this.guiTop + sortButtonY + 3 * (buttonHeight + 1),
                -1 + (buttonWidth) / 4,
                buttonHeight,
                ">>"));
    ...
    private void initSeedSlots() {
        ...
        if (list != null) {
            ...
            for (int i = scrollPositionHorizontal; i
                < Math.min(list.size(), scrollPositionHorizontal + maxHorSlots); i++) {
                SeedStorageSlot slot = list.get(i);
                PlantStatsStorage stats = new PlantStatsStorage(
                    slot.getId(),
                    slot.getStack(this.activeSeed, this.activeMeta));
                activeSeeds.add(
                    new Component<>(
                        stats,
                        this.guiLeft + seedSlotButtonOffset_X + (i - scrollPositionHorizontal) * 16,
                        this.guiTop + seedSlotButtonOffset_Y,
                        16,
                        16));
            }
        }
        ...
        if (this.hasActiveSeed()) {
            for (Component<PlantStatsStorage> component : activeSeeds) {
                if (component.isOverComponent(x, y)) {
                    ...
                    this.container
                        .moveStackFromTileEntityToPlayer(stats.id, new ItemStack(activeSeed, stackSize, activeMeta));
                    this.updateScreen();
                    return;
                }
            }
    ...
    private int seedSlotAmount() {
        return this.hasActiveSeed() ? this.container.getSeedSlots(activeSeed, activeMeta)
            .size() : 0;
    }
