...
public class GuiPeripheral extends GuiContainer {
    ...
    public static final ResourceLocation texture = new ResourceLocation(
        ...
        "textures/gui/GuiPeripheral.png");
    ...
    public void drawGuiContainerForegroundLayer(int x, int y) {
        String name = StatCollector.translateToLocal("cropsnh_gui.peripheral");
        // write name: x coordinate is in the middle, 6 down from the top, and setting color to white
        float scale = 0.8F;
        GL11.glScalef(scale, scale, scale);
        this.fontRendererObj.drawString(
            name,
            (int) ((9 + this.xSize / 2 - this.fontRendererObj.getStringWidth(name) / 2) / scale),
            (int) (6 / scale),
            WHITE);
    ...
    private void drawMethodHelp(IMethod method) {
        if (method != null) {
            ...
            this.fontRendererObj.drawString(
                StatCollector.translateToLocal("cropsnh_description.peripheralHelp") + ": " + method.signature(),
                this.guiLeft + 7,
                this.guiTop + 175,
                WHITE);
            float scale = 0.9F;
            GL11.glScalef(scale, scale, scale);
            String[] write = IOHelper.getLinesArrayFromData(
                IOHelper.splitInLines(this.fontRendererObj, method.getDescription(), 230, scale));
            int x = 4 + this.guiLeft + 7;
            int y = this.guiTop + 175 + height;
            for (int i = 0; i < write.length; i++) {
                String line = write[i];
                int yOffset = i * height;
                this.fontRendererObj.drawString(line, (int) (x / scale), (int) (y / scale) + yOffset, WHITE); // 1644054
                                                                                                              // means
                                                                                                              // black
            }
            GL11.glScalef(1 / scale, 1 / scale, 1 / scale);
    ...
    private void loadButtonList() {
        ...
        this.buttonList
            .add(new GuiButton(BUTTON_ID_SCROLL_TOP, this.guiLeft + 154, this.guiTop + 20, 10, 10, "\u219F"));
        this.buttonList.add(new GuiButton(BUTTON_ID_SCROLL_UP, this.guiLeft + 154, this.guiTop + 31, 10, 10, "\u2191"));
        this.buttonList
            .add(new GuiButton(BUTTON_ID_SCROLL_DOWN, this.guiLeft + 154, this.guiTop + 42, 10, 10, "\u2193"));
        this.buttonList
            .add(new GuiButton(BUTTON_ID_SCROLL_BOTTOM, this.guiLeft + 154, this.guiTop + 53, 10, 10, "\u21A1"));
        for (int i = 0; i < methods.length; i++) {
            this.buttonList.add(
                new GuiButtonMethod(
                    BUTTON_METHOD_OFFSET + i,
                    this.guiLeft + guideOffset + 3,
                    this.guiTop + 8 + 16 * i,
                    68,
                    16,
                    methods[i].getName()));
        ...
        public void drawButton(Minecraft minecraft, int x, int y) {
            if (this.visible) {
                ...
                this.drawCenteredString(
                    fontrenderer,
                    this.displayString,
                    (int) ((this.xPosition + this.width / 2) / scale),
                    (int) ((this.yPosition + (this.height - 4) / 2) / scale),
                    l);
                GL11.glScalef(1 / scale, 1 / scale, 1 / scale);
            }
        }
