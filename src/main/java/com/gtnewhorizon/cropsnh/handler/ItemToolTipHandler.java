...
public class ItemToolTipHandler {
        ...
        if(CropPlantHandler.isValidSeed(stack) && stack.hasTagCompound()) {
            ...
            if(tag.hasKey(Names.NBT.growth) && tag.hasKey(Names.NBT.gain) && tag.hasKey(Names.NBT.strength) && tag.hasKey(Names.NBT.analyzed)) {
                if(tag.getBoolean(Names.NBT.analyzed)) {
                    event.toolTip.add(EnumChatFormatting.GREEN + " - "+StatCollector.translateToLocal("cropsnh_tooltip.growth") + ": " + StatStringDisplayer.instance().getStatDisplayString(tag.getInteger(Names.NBT.growth), ConfigurationHandler.cropStatCap));
                    event.toolTip.add(EnumChatFormatting.GREEN + " - "+StatCollector.translateToLocal("cropsnh_tooltip.gain") + ": " + StatStringDisplayer.instance().getStatDisplayString(tag.getInteger(Names.NBT.gain), ConfigurationHandler.cropStatCap));
                    event.toolTip.add(EnumChatFormatting.GREEN + " - "+StatCollector.translateToLocal("cropsnh_tooltip.strength") + ": " + StatStringDisplayer.instance().getStatDisplayString(tag.getInteger(Names.NBT.strength), ConfigurationHandler.cropStatCap));
                }
                else {
    ...
    public void addTrowelTooltip(ItemTooltipEvent event) {
        ...
        if(stack.getItemDamage()==0) {
            ...
            event.toolTip.add(StatCollector.translateToLocal("cropsnh_tooltip.seed") + ": " + seed.getItem().getItemStackDisplayName(seed));
        }
    }
