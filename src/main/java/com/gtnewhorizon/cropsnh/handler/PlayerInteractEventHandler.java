    ...
    public void waterPadCreation(PlayerInteractEvent event) {
        if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
            ...
            if (!ConfigurationHandler.onlyCreateWaterPadWhileSneaking || event.entityPlayer.isSneaking()) {
                boolean flag = false;
                if (event.entityPlayer.getCurrentEquippedItem() != null && event.entityPlayer.getCurrentEquippedItem()
                    .getItem() != null
                    && event.entityPlayer.getCurrentEquippedItem()
                ...
                if (flag) {
                    ...
                    event.world
                        .setBlock(event.x, event.y, event.z, com.gtnewhorizon.cropsnh.init.Blocks.blockWaterPad, 0, 3);
                    if (!event.entityPlayer.capabilities.isCreativeMode) {
                        event.entityPlayer.getCurrentEquippedItem()
                            .damageItem(1, event.entityPlayer);
    ...
    private void denyEvent(PlayerInteractEvent event, boolean sendToServer) {
        ...
        if (sendToServer && event.world.isRemote) {
            ...
            FMLClientHandler.instance()
                .getClientPlayerEntity().sendQueue.addToSendQueue(
