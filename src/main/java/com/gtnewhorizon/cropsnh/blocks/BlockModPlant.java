...
public class BlockModPlant extends BlockCrops implements ICropsNHPlant {
    ...
    /**
     * Parameters can be given in any order, parameters can be: @param args: Arguments can be given in any order,
     * parameters can be: String name (needed) ItemStack fruit(needed) BlockWithMeta soil (optional, pass this argument
     * before the RequirementType, else it will be interpreted as a baseblock) RequirementType (optional) BlockWithMeta
     * baseBlock (optional, only if a RequirementType is specified first, else it will be set a a soil) int tier
     * (necessary) RenderMethod renderType (necessary) ItemStack shearDrop (optional, first ItemStack argument will be
     * the regular fruit, second ItemStack argument is the shear drop) int[] brightness (optional, if not given it will
     * default to {8, 16}. Only works if the array is size 2: {minBrightness, maxBrightness}) Will throw
     * MissingArgumentsException if the needed arguments are not given. This constructor creates the seed for this plant
     * which can be gotten via blockModPlant.getSeed(). This constructor also registers this block and the item for the
     * seed to the minecraft item/block registry and to the CropsNH CropPlantHandler.
     */
    public BlockModPlant(Object... arguments) throws MissingArgumentsException {
        ...
        this.seed = new ItemModSeed(
                this,
                "cropsnh_journal." + Character.toLowerCase(name.charAt(0)) + name.substring(1));
    ...
    public void registerBlockIcons(IIconRegister reg) {
        ...
        for (int i = 1; i < this.icons.length + 1; i++) {
            this.icons[i - 1] = reg
                    .registerIcon(this.getUnlocalizedName().substring(this.getUnlocalizedName().indexOf('.') + 1) + i);
        }
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random rnd) {
        ...
        if (meta < Constants.MATURE && this.isFertile(world, x, y, z)) {
            // Base growth rate
            int growthRate = (tier > 0 && tier <= Constants.GROWTH_TIER.length) ? Constants.GROWTH_TIER[tier]
                    : Constants.GROWTH_TIER[0];
