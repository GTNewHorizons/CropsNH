...
public class BlockModPlant extends BlockCrops implements ICropsNHPlant {
     ...
     * BlockWithMeta soil (optional, pass this argument before the RequirementType, else it will be interpreted as a
     * baseblock)
     ...
     * ItemStack shearDrop (optional, first ItemStack argument will be the regular fruit, second ItemStack argument is
     * the shear drop)
     * int[] brightness (optional, if not given it will default to {8, 16}. Only works if the array is size 2:
     ...
     * This constructor also registers this block and the item for the seed to the minecraft item/block registry and to
     * the CropsNH CropPlantHandler.
     */
    public BlockModPlant(Object... arguments) throws MissingArgumentsException {
        ...
        if (soil == null) {
            ...
            growthRequirement = builder.soil(soil)
                .build();
        }
    ...
    public void registerBlockIcons(IIconRegister reg) {
        ...
        for (int i = 1; i < this.icons.length + 1; i++) {
            this.icons[i - 1] = reg.registerIcon(
    ...
    protected Item func_149865_P() {
        Item randomFruit = this.getRandomFruit(new Random())
            .getItem();
